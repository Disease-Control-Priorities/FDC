...
HPACC<-read.csv("ST_ASA_HPACC_SP.csv", stringsAsFactors = F)%>%
  mutate(location_name = ifelse(location_name=="Iran", "Iran (Islamic Republic of)", location_name),
         location_name = ifelse(location_name=="Moldova", "Republic of Moldova", location_name),
         ...
         location_name = ifelse(location_name=="St. Vincent & the Grenadines", "Saint Vincent and the Grenadines", location_name),
         source="HPACC")
...
# with updated data, the prediction is now %5 (with HPACC)
...
baseline.coverage<-left_join(baseline.coverage, cascade%>%select(location_name, iso3, pr_trt, r_HTN_pp))%>%
  ...
  mutate(pp_cov = ifelse(cause=="ihd", r_HTN_pp*(0.80*statin.pp + 0.17*pr_trt),
                     ifelse(cause=="istroke", r_HTN_pp*(0.5*statin.pp + 0.50*pr_trt),
                            r_HTN_pp*(pr_trt))),
         sp_cov = ifelse(cause=="ihd", r_IHD*(0.16*statin.sp.ihd + 0.18*aspirin.sp.ihd + 0.31*bbl.sp.ihd+0.35*ras.sp.ihd),
                         ifelse(cause=="istroke", r_stroke*(ras.sp.cva),
                                NA)) #not doing hstroke for now
         )
...
ggplot(plot, aes(x=ldi_pc, y=pp_cov, color=wb2021))+
  geom_point()+
  facet_wrap(~cause)+
  ggtitle("Primary prevention coverage")
...
ggplot(plot, aes(x=ldi_pc, y=sp_cov, color=wb2021))+
  geom_point()+
  facet_wrap(~cause)+
  ggtitle("Secondary prevention coverage")
...
targets<-merge(x = c("Baseline", "Scenario 1", "Scenario 2", "Scenario 3", "Scenario 4"),
  ...
  mutate(PP_eff = ifelse(cause == "ihd", 1-0.59,
                         ifelse(cause=="istroke", 1-0.62,
                                ifelse(cause=="hstroke", 1-0.47,0))),
         SP_eff = ifelse(cause == "ihd", 1-0.73,
                         ifelse(cause=="istroke", 1-0.73,0)),
         cov_inc = ifelse(intervention == "Baseline", 0,
                          ifelse(intervention=="Scenario 4", 0.045,
                                 ifelse(intervention=="Scenario 3", 0.025, 0.015)))
  )
...
df_out<-left_join(targets, prop.cov)%>%
  ...
  mutate(pp.cov.inc = ifelse(year>=2023, cov_inc*(year-2022), 0),
         ...
         pp.max = ifelse(intervention == "Baseline", pr_trt,
                      ifelse(intervention == "Scenario 1", pr_trt,
                             ifelse(intervention== "Scenario 2", pr_ddx, 0.8))), #cap at Scenario-specific targets
         sp.max = ifelse(intervention == "Baseline", pr_trt_SP,
                         ifelse(intervention == "Scenario 1", pr_trt_SP,
                                ifelse(intervention== "Scenario 2", 0.7, 0.8))),
...
ggplot(df_out%>%filter(cause=="ihd"), aes(x=year, y=pp.cov.inc+pp_cov, group=location_name))+
  ...
  xlab("Year")+
  ggtitle("Primary prevention coverage scale-up: IHD")

ggsave("PP_cov_ihd.jpeg", height=4, width=12)
...
ggsave("SP_cov_ihd.jpeg", height=4, width=12)
