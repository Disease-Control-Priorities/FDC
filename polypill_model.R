...
project.all <- function(Country){
  ...
  intervention_rates[,PP:=1-(PP_eff*pp.cov.inc)]
  intervention_rates[,SP:=1-(SP_eff*sp.cov.inc)]
...
time2<-Sys.time()
time2-time1 #32 mins for 181 countries

write.csv(output, "output.csv", row.names = F)

#########################################################################
output<-read.csv("output.csv", stringsAsFactors = F)
#Tables and figures

unique(output%>%filter(is.na(intervention))%>%pull(location))

table1<-output%>%
  filter(cause!="hhd")%>%
  group_by(intervention,year)%>%
  summarise(Incidence = signif(sum(newcases),2),
            Prevalence = signif(sum(sick),2),
            CVD_deaths=signif(sum(dead),2),
            All_deaths = signif(sum(all.mx)/3,2))%>%
...
write.csv(table1_b, "Table2_b.csv")
...
write.csv(table2, "cumulative_resuts.csv")
...
ggsave("Figure1.jpeg", height=4, width=9)

write.csv(WB_50q30, "Fig1_data.csv")
...
ggsave("Figure2_area.jpeg", height=4, width=9)

#Compared to the business-as-usual scenario, the cumulative number of CVD deaths and cases that could be averted (2023-2050) 
#would be ## and ## million in scenario 1, ## and ## million in scenario 2, ## and ## million in scenario 3, and ## and ## million in scenario 4.
